...
std::vector<range> find_substring(const wide_string large_str,
    ...
    const auto large_size = large_str.size();
    std::vector<full_finder> v_find(large_size + 1);
    for (int c = 0; c < large_size + 1; c++) {
        v_find[c].set[small_str.size() & 1].end_at = c;
    }
    for (signed long cs = small_str.size() - 1; cs >= 0; cs--) {
        int now = cs & 1;
        int last = now ^ 1;
        for (signed long cl = large_str.size() - 1; cl >= 0; cl--) {
        ...
        v_find[large_size].set[now].distance = (int)(small_str.size() - cs);
        v_find[large_size].set[now].end_at = (int)large_size;
    }
